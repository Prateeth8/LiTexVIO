...
try:
    ...
    raise SystemExit("Open3D is not installed. Run: pip3 install open3d")
...
def run_icp():
    files = sorted(glob.glob(os.path.join(PCL_DIR, "scan_*.npy")))
    ...
    previous_cloud = previous_cloud.voxel_down_sample(voxel_size=0.08)

    for i in range(1, len(files)):
        current = np.load(files[i])
        current = current[np.isfinite(current).all(axis=1)]
        current_cloud = make_cloud(current)
        current_cloud = current_cloud.voxel_down_sample(voxel_size=0.08)
        ...
        trajectory.append([x, y, yaw_from_rotation(accumulated[:3, :3])])
        results.append([i, dx, dy, dz, dyaw, result.fitness, result.inlier_rmse])
    ...
    df = pd.DataFrame(
        results,
        columns=["scan_id", "dx", "dy", "dz", "dyaw", "fitness", "rmse"]
    )
...
def integrate_icp_with_ekf(icp_traj, ekf):
    ...
    ekf_index = np.linspace(0, len(ekf) - 1, len(icp_traj))
    target_index = np.arange(len(ekf))

    lidar_x = np.interp(target_index, ekf_index, icp_x)
    lidar_y = np.interp(target_index, ekf_index, icp_y)
    lidar_yaw = np.interp(target_index, ekf_index, icp_yaw)

    alpha = 0.65

    fused_x = alpha * ex + (1.0 - alpha) * lidar_x
    fused_y = alpha * ey + (1.0 - alpha) * lidar_y
    fused_yaw = alpha * eyaw + (1.0 - alpha) * lidar_yaw
    ...
    result.to_csv(FINAL_EKF_FILE, index=False)
...
def plot_all(odom, noisy, ekf, fused):
    ...
    plt.axvspan(10, 15, alpha=0.12, label="Slip region")

    plt.xlabel("X (m)")
    plt.ylabel("Y (m)")
    plt.title("Trajectory comparison: noisy sensor fusion with LiDAR")
    plt.legend()
    plt.grid(True)
    plt.axis("equal")
    plt.tight_layout()
...
def main():
    ...
    fused = integrate_icp_with_ekf(icp_traj, ekf)

    print("Creating final plot...")
    plot_all(odom, noisy, ekf, fused)
